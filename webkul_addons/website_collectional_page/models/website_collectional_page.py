...
class WebsiteCollectionalPage(models.Model):
    ...
    #         if self.option == "conditionally" and not vals.has_key("template_ids"):
    #             product_objs = self.get_all_products()
    #             _logger.info(
    ...
    #         base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
    #         base_url = base_url + "/collections/"
    #         vals['url'] = base_url + (vals.has_key("url_handler")
    ...
    def write(self, vals):
        ...
        for obj in self:
            vals.clear()
            if obj.option == "conditionally" and not vals.has_key("template_ids"):
...
class ProductCondition(models.Model):
    ...
    def _product_field_get(self):
        ids = self.env['ir.model.fields'].search([('model', 'in', ['product.template']), ('name', 'in', [
