...
class ProductTemplate(models.Model):
    ...
    def _check_commission_values(self, vals):
        for rec in self:
            ...
            _logger.info("-------TEST--------%r------------------%r-",public_categ_ids, isinstance(public_categ_ids, list))
            if isinstance(public_categ_ids, list):
                if public_categ_ids and public_categ_ids[0] and len(public_categ_ids) >= 2 and public_categ_ids[0][2]:
            ...
            seller_obj = vals.get('marketplace_seller_id') if vals.get('marketplace_seller_id') else rec.marketplace_seller_id
            if seller_obj:
        ...
        if comm_method:
            ...
            if comm_method == 'fix' or comm_method == 'percent_and_fix' or comm_method == 'fix_and_percent':
                if fix_comm > list_price:
                    if category:
                        raise ValidationError(_('Value of Fixed Commission defined in selected Category should be less than sale price of product.'))
                    elif seller:
                        raise ValidationError(_('Value of Fixed Commission defined in  seller profile should be less than sale price of product.'))
                    else:
                        raise ValidationError(_('Value of Fixed Commission of product should be less than sale price of product.  '))
            ...
            if comm_method == 'fix_and_percent' :
                ...
                if comm_factor > list_price:
                    if category:
                        raise ValidationError(_('Value of Fixed + %  Commissiond in selected Category define should be less than sale price of product.'))
                    elif seller:
                        raise ValidationError(_('Value of Fixed + %  Commission defined in Seller profile should be less than sale price of product.'))
                    else:
                        raise ValidationError(_('Value of Fixed + %  commission of product should be less than sale price of product.'))
