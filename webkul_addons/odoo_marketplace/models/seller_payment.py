...
class seller_payment(models.Model):
    ...
    def _get_mp_currency(self):
        return self.env['ir.values'].get_default('marketplace.config.settings', 'mp_currency_id') or self.env.user.company_id.currency_id
    ...
    date = fields.Date(
        string="Payment Date", default=datetime.datetime.today(), required=True, readonly=True, states={'draft': [('readonly', False)]}, copy=False)
    ...
    def _set_invoice_currency(self):
        self.invoice_currency_id = self.invoice_id.currency_id or self.env['ir.values'].get_default('marketplace.config.settings', 'mp_currency_id') or self.env.user.company_id.currency_id

    # @api.one
    # @api.depends("company_id", "company_id.currency_id")
    # def _set_mp_currency(self):
    #     self.currency_id = self.env['ir.values'].get_default('marketplace.config.settings', 'mp_currency_id') or self.env.user.company_id.currency_id
    ...
    def create(self, vals):
        ...
        res = super(seller_payment, self).create(vals)
        return res
    ...
    def write(self, vals):
        ...
        if vals.get("payable_amount", False) or vals.get("payment_type", False):
            ...
            if payment_type == 'dr':
                ...
                vals.update({'payable_amount': abs(payable_amount)})
        res = super(seller_payment, self).write(vals)
        return res
    ...
    def unlink(self):
        ...
        return super(seller_payment, self).unlink()
    ...
    def _needaction_domain_get(self):
        return [('payment_type', '=', 'dr'), ('state', 'in', ['confirm', 'requested'])]
