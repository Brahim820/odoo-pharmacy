...
class ResPartner(models.Model):
    ...
    commission = fields.Float(string="Default Sale Commission", default=lambda self: self.env['ir.values'].get_default(
        'marketplace.config.settings', 'global_commission'), read=['odoo_marketplace.marketplace_draft_seller_group'], write=['odoo_marketplace.marketplace_manager_group'], copy=False)
    ...
    location_id = fields.Many2one("stock.location", string="Default Location", domain="[('usage', '=', 'internal')]", default=lambda self: self.env[
                                  'ir.values'].get_default('marketplace.config.settings', 'warehouse_location_id') or self.env["stock.location"], copy=False)
    auto_product_approve = fields.Boolean(string="Auto Product Approve", default=lambda self: self.env[
                                          'ir.values'].get_default('marketplace.config.settings', 'auto_product_approve'), copy=False)
    seller_payment_limit = fields.Integer(string="Seller Payment Limit", default=lambda self: self.env['ir.values'].get_default(
        'marketplace.config.settings', 'seller_payment_limit'), read=['odoo_marketplace.marketplace_draft_seller_group'], write=['odoo_marketplace.marketplace_manager_group'], copy=False)
    next_payment_request = fields.Integer(string="Next Payment Request", default=lambda self: self.env['ir.values'].get_default(
        'marketplace.config.settings', 'next_payment_requset'), read=['odoo_marketplace.marketplace_draft_seller_group'], write=['odoo_marketplace.marketplace_manager_group'], copy=False)
    auto_approve_qty = fields.Boolean(string="Auto Quantity Approve", default=lambda self: self.env[
                                      'ir.values'].get_default('marketplace.config.settings', 'auto_approve_qty'), copy=False)
    ...
    show_seller_since = fields.Boolean(string="Show Seller Since", default=lambda self: self.env[
                                       'ir.values'].get_default('marketplace.config.settings', 'seller_since'), copy=False)
    show_seller_address = fields.Boolean(string="Show Seller Address", default=lambda self: self.env[
                                         'ir.values'].get_default('marketplace.config.settings', 'shipping_address'), copy=False)
    show_product_count = fields.Boolean(string="Show Product Count", default=lambda self: self.env[
                                        'ir.values'].get_default('marketplace.config.settings', 'product_count'), copy=False)
    show_sale_count = fields.Boolean(string="Show Sale Count", default=lambda self: self.env[
                                     'ir.values'].get_default('marketplace.config.settings', 'sale_count'), copy=False)

    show_return_policy = fields.Boolean(string="Show Return Policy", default=lambda self: self.env[
                                        'ir.values'].get_default('marketplace.config.settings', 'return_policy'), copy=False)
    show_shipping_policy = fields.Boolean(string="Show Shipping Policy", default=lambda self: self.env[
                                          'ir.values'].get_default('marketplace.config.settings', 'shipping_policy'), copy=False)
    show_seller_review = fields.Boolean(string="Show Seller Reviews", default=lambda self: self.env[
        'ir.values'].get_default('marketplace.config.settings', 'seller_review'), copy=False)
    ...
    status_msg = fields.Text(string="Account Status Message", translate=True, default=lambda self: self.env["marketplace.config.settings"].mp_config_translatable().get("seller_new_status_msg", "Thank you for registering with us, to enjoy the benefits of our marketplace fill all your details and request for approval."))
    ...
    manager_fields.extend(['commission','seller_payment_limit','next_payment_request','auto_product_approve',
        'auto_approve_qty','location_id','warehouse_id','show_seller_since','show_product_count','show_sale_count',
        'show_seller_review','allow_product_variants','show_seller_address','show_return_policy','show_shipping_policy',
    ...
    def _get_seller_currency(self):
        for obj in self:
            obj.seller_currency_id = self.env['ir.values'].get_default('marketplace.config.settings', 'mp_currency_id') or self.env.user.company_id.currency_id
    ...
    def on_change_location_id(self):
        ...
        if whs:
            ...
            self.warehouse_id = self.env["stock.warehouse"]

    @api.onchange("set_seller_wise_settings")
    def on_change_seller_wise_settings(self):
        if not self.set_seller_wise_settings:
            vals = {}
            vals["commission"] = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'global_commission')
            vals["location_id"] = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'warehouse_location_id')
            vals["auto_product_approve"] = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'auto_product_approve')
            vals["seller_payment_limit"] = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'seller_payment_limit')
            vals["next_payment_request"] = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'next_payment_requset')
            vals["auto_approve_qty"] = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'auto_approve_qty')
            vals["show_seller_since"] = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'seller_since')
            vals["show_seller_address"] = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'shipping_address')
            vals["show_product_count"] = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'product_count')
            vals["show_sale_count"] = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'sale_count')
            vals["show_return_policy"] = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'return_policy')
            vals["show_shipping_policy"] = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'shipping_policy')
            vals["show_seller_review"] = self.env['ir.values'].get_default(
    ...
    def on_change_seller(self):
        ...
        self.commission = config_setting_obj[
            "global_commission"] if config_setting_obj.has_key("global_commission") else False
        self.warehouse_id = config_setting_obj[
            "default_warehouse"] if config_setting_obj.has_key("default_warehouse") else False
        self.location_id = config_setting_obj[
            "warehouse_location_id"] if config_setting_obj.has_key("warehouse_location_id") else False
        self.auto_product_approve = config_setting_obj[
            "auto_product_approve"] if config_setting_obj.has_key("auto_product_approve") else False
        self.seller_payment_limit = config_setting_obj[
            "seller_payment_limit"] if config_setting_obj.has_key("seller_payment_limit") else 0
        self.next_payment_request = config_setting_obj[
            "next_payment_requset"] if config_setting_obj.has_key("next_payment_requset") else 0
        self.auto_approve_qty = config_setting_obj[
            "auto_approve_qty"] if config_setting_obj.has_key("auto_approve_qty") else False
    ...
    def set_to_pending(self):
        if self.seller:
            ...
            if config_setting_obj.get("auto_approve_seller", False):
                ...
                self.status_msg = self.env["marketplace.config.settings"].mp_config_translatable().get("seller_pending_status_msg", "Thank you for seller request, your request has been already sent for approval we'll process your request as soon as possible.")
    ...
    def change_seller_group(self, set_to_group=None):
        """ param: set_to_group should be string, value can be 'not_seller' or 'seller' only. """
        if not set_to_group:
            return
    ...
    def change_seller_group_and_send_mail(self):
        ...
        if self.seller and self.state == "approved":
            ...
            config_setting_obj = self.env['marketplace.config.settings'].sudo().get_default_values()
            if config_setting_obj.get("enable_notify_admin_on_seller_approve_reject", False) and config_setting_obj.get("notify_admin_on_seller_approve_reject", False):
                # Notify to admin by admin on new seller creation
                self.notify_via_mail_to_seller(config_setting_obj.get("notify_admin_on_seller_approve_reject", False))
            if config_setting_obj.get("enable_notify_seller_on_approve_reject", False) and config_setting_obj.get("notify_seller_on_approve_reject", False):
                # Notify to Seller by admin on new seller creation
                self.notify_via_mail_to_seller(config_setting_obj.get("notify_seller_on_approve_reject", False))
            ...
            if self._context.get("new_to_pending", False):
                #Send mail to admin/seller on seller approval
                config_setting_obj = self.env['marketplace.config.settings'].sudo().get_default_values()
                if config_setting_obj.get("enable_notify_admin_4_new_seller", False) and config_setting_obj.get("notify_admin_4_new_seller", False):
                    # Notify to admin by admin on new seller creation
                    self.notify_via_mail_to_seller(config_setting_obj.get("notify_admin_4_new_seller", False))
                if config_setting_obj.get("enable_notify_seller_4_new_seller", False) and config_setting_obj.get("notify_seller_4_new_seller", False):
                    # Notify to Seller by admin on new seller creation
                    self.notify_via_mail_to_seller(config_setting_obj.get("notify_seller_4_new_seller", False))
            else:
                #Send mail to admin/seller on seller approval
                config_setting_obj = self.env['marketplace.config.settings'].sudo().get_default_values()
                if config_setting_obj.get("enable_notify_admin_on_seller_approve_reject", False) and config_setting_obj.get("notify_admin_on_seller_approve_reject", False):
    ...
    def get_limit(self):
        config_setting_obj = self.env[
            'marketplace.config.settings'].sudo().get_default_values()
        if config_setting_obj.has_key('review_load_no'):
            return config_setting_obj['review_load_no']
        else:
            return 0
    ...
    def seller_products_count(self):
        ...
        all_products = self.env['product.template'].sudo().search(
            ...
            ("sale_ok", "=", True), ("is_mobikul_available", "=", True)])
